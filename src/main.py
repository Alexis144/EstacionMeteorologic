...
import time
import threading 
from db import leer_datos, insertar_dato
...
def main(page: ft.Page):
    ...
    def update_ui(e=None):
        ...
        temp_card.controls = [data_card("Temp DHT22", "🌡", temp, "°C", "#f90808", "#0056d6", temp_alert)]
        hum_card.controls = [data_card("Humedad", "💧", hum, "%", "#a0d0ea", "#2980b9", hum_alert)]
        pres_card.controls = [data_card("Presión", "🌬", pres, "hPa", "#96a3a9", "#607d8b")]
        luz_card.controls = [data_card("Luz", "💡", lux, "lux", "#ff5100", "#fdd835")]
        lluvia_card.controls = [data_card("Lluvia", "🌧" if rain else "☀", "Sí" if rain else "No", "", "#0000f7", "#0288d1", rain_alert)]
    ...
    page.add(ft.Column(
        ...
        controls=[
            ft.Text("🛰 SCADA - Estación Meteorológica", size=28, weight=ft.FontWeight.BOLD, color="white"),reloj,
            ft.Text("ALERTAS", size=16, color="white70"), alert_container,
